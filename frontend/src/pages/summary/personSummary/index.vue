<template>
  <div class="new-page" :style="`min-height: ${pageMinHeight}px`">
    ...
    <detial-modal v-model="visible" :person="targetPerson" />
  </div>
</template>

<script>
import { mapState } from "vuex";
import { personSummary, personSummaryExport, assetSummaryExport } from "@/api/summary";
...
export default {
  ...
  methods: {
    ...
    exportData() {
      personSummaryExport(this.searchForm).then((resp) => {
        exportExcel(resp, "人员汇总");
      });
    },
    ...
    exportDetial(item) {
      ...
      assetSummaryExport({ person: item.person, status }).then((resp) => {
        exportExcel(resp, `人员汇总-${item.person_name}`);
      });
    },
    openDetialModal(item) {
      this.targetPerson = item.person;
      this.visible = true;
    },
  },
  mounted() {
